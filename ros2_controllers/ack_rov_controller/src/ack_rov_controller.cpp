...
#include "ack_rov_controller/ack_rov_controller.hpp"
...
AckRovController::AckRovController() : controller_interface::ControllerInterface() {}

const char * AckRovController::feedback_type() const
{
  return odom_params_.position_feedback ? HW_IF_POSITION : HW_IF_VELOCITY;
}

const char * AckRovController::feedback_type_steer() const
{
  return  HW_IF_POSITION ;
}

CallbackReturn AckRovController::on_init()
{
  try
  {
...
InterfaceConfiguration AckRovController::command_interface_configuration() const
{
...
InterfaceConfiguration AckRovController::state_interface_configuration() const
{
  std::vector<std::string> conf_names;
  for (const auto & joint_name : left_wheel_names_)
  {
  ...
  for (const auto & joint_name : left_steering_names_)                    
  {
    conf_names.push_back(joint_name + "/" + feedback_type_steer());
  }
  for (const auto & joint_name : right_steering_names_)
  {
    conf_names.push_back(joint_name + "/" + feedback_type_steer());
...
controller_interface::return_type AckRovController::update(
  const rclcpp::Time & time, const rclcpp::Duration & /*period*/)
{
  ...
  double desired_turning_radius;
  double wheel_base = 0.1;
  double wheel_track = 0.208;
 
    
/////////////////////////// compute Turning radius  //////////////////////////////////////////

if(command.twist.linear.x <0)
{
   desired_turning_radius = command.twist.linear.x / -command.twist.angular.z;
}
else
{
   desired_turning_radius = command.twist.linear.x / command.twist.angular.z;
}
if(command.twist.angular.z == 0)
{
    desired_turning_radius = max_radius;
}


///////////////////////////////// compute steering angle/////////////////
double theta_front_closest = atan2(wheel_base, abs(desired_turning_radius) - (wheel_track/2));
double theta_front_farthest = atan2(wheel_base, abs(desired_turning_radius) + (wheel_track/2));

//////////////////////// set steering angle///////////////////////////////

if(desired_turning_radius > 0)   // turning left
{
  registered_left_steer_handles_[0].position.get().set_value(theta_front_closest);
  registered_left_steer_handles_[1].position.get().set_value(-theta_front_closest);
  registered_right_steer_handles_[0].position.get().set_value(theta_front_farthest);
  registered_right_steer_handles_[1].position.get().set_value(-theta_front_farthest);

}

else  // turning right
{
  registered_left_steer_handles_[0].position.get().set_value(-theta_front_farthest);
  registered_left_steer_handles_[1].position.get().set_value(theta_front_farthest);
  registered_right_steer_handles_[0].position.get().set_value(-theta_front_closest);
  registered_right_steer_handles_[1].position.get().set_value(theta_front_closest);

}
if(isnan(desired_turning_radius))   
{
  desired_turning_radius = 0;

  registered_left_steer_handles_[0].position.get().set_value(0);
  registered_left_steer_handles_[1].position.get().set_value(0);
  registered_right_steer_handles_[0].position.get().set_value(0);
  registered_right_steer_handles_[1].position.get().set_value(0);

}


RCLCPP_INFO(logger, "The Radius is [%f]: ",desired_turning_radius); 
 

  return controller_interface::return_type::OK;
}

CallbackReturn AckRovController::on_configure(const rclcpp_lifecycle::State &)
{
  auto logger = node_->get_logger();
  ...
  {
    RCLCPP_ERROR(
      logger, "The number of left steering joints [%zu] and the number of right steering joints [%zu] are different",
  ...
  {
    RCLCPP_ERROR(logger, "Steering joint names parameters are empty!");
    return CallbackReturn::ERROR;
  }
...
CallbackReturn AckRovController::on_activate(const rclcpp_lifecycle::State &)
{
  ...
  const auto left_result_steer =                                                                        
    configure_side_steer("left", left_steering_names_, registered_left_steer_handles_);
  const auto right_result_steer =
    configure_side_steer("right", right_steering_names_, registered_right_steer_handles_);
  ...
  if (left_result_steer == CallbackReturn::ERROR || right_result_steer == CallbackReturn::ERROR)          
  {
    return CallbackReturn::ERROR;
  }
  ...
  if (registered_left_steer_handles_.empty() || registered_right_steer_handles_.empty())               
  {
    RCLCPP_ERROR(
      node_->get_logger(), "Either left steer joint interfaces, right steer joint  interfaces are non existent");
    return CallbackReturn::ERROR;
  }
...
CallbackReturn AckRovController::on_deactivate(const rclcpp_lifecycle::State &)
{
  subscriber_is_active_ = false;
  return CallbackReturn::SUCCESS;
}

CallbackReturn AckRovController::on_cleanup(const rclcpp_lifecycle::State &)
{
  if (!reset())
  {
    return CallbackReturn::ERROR;
...
CallbackReturn AckRovController::on_error(const rclcpp_lifecycle::State &)
{
  if (!reset())
  {
    return CallbackReturn::ERROR;
...
CallbackReturn AckRovController::on_shutdown(const rclcpp_lifecycle::State &)
{
  return CallbackReturn::SUCCESS;
}

void AckRovController::halt()
{
...
CallbackReturn AckRovController::configure_side(
...
CallbackReturn AckRovController::configure_side_steer(                                 
  ...
  std::vector<SteerHandle> & registered_handles)
{
  ...
  {
    RCLCPP_ERROR(logger, "No '%s' steer joint names specified", side.c_str());
    return CallbackReturn::ERROR;
  }

  // register handles
  registered_handles.reserve(steering_names.size());
  for (const auto & steer_name : steering_names)
  {
    const auto interface_name = feedback_type();
    const auto state_handle = std::find_if(
      state_interfaces_.cbegin(), state_interfaces_.cend(),
      [&steer_name, &interface_name](const auto & interface) {
        return interface.get_name() == steer_name &&
               interface.get_interface_name() == interface_name;
      });
...
CLASS_LOADER_REGISTER_CLASS(
  ack_rov_controller::AckRovController, controller_interface::ControllerInterface)
